...
if(isset($_SESSION['adminloggedin'])) {
    ...
    if(isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO sales(lotNumber, pieceTitle, commissionBids, reservePrice, sold, price, clientNumber, auctionComments  ) 
                                       VALUES(:lotNumber,:pieceTitle, :commissionBids, :reservePrice, :sold, :price, :clientNumber, :auctionComments)
    ');
    ...
    //if the submit button is not pressed the empty form is displayed
        ...
        <form action="addSales.php" method="POST">
            ...
            <label>Sold </label> <input name = "sold" type="text" required/>
            <label>Price</label> <input name = "price" type="text" required/>
            <label>Client Number</label> <input name = "clientNumber" type="text" required/>
